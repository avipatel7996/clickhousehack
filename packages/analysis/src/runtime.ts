...
export async function runAnalysis(question: string, options: AnalysisRuntimeOptions): Promise<StructuredAnalysisAnswer> {
  ...
  let calls = 0;
  while (true) {
    const response = await options.client.chat({ messages, model: options.model, temperature: 0, response_format: { type: "json_object" }, tools: queryTool ? [queryTool] : [], tool_choice: "auto" }, options.signal);
